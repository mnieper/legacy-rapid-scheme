...
(define (graph-scc neighbors-alist vertex-comparator)
  ...
  (define (pop!)
    (let ((vertex (car stack)))
      ...
      vertex))  
  (define vertices (make-table vertex-comparator))
  (define (intern-vertex! vertex)
    (define (failure) (make-vertex vertex))
    (table-intern! vertices vertex failure))
  (define (ref-vertex vertex)
    (table-ref vertices vertex))
  (define sccs '())
